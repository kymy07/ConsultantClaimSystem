...
not even drawn for those accounts — the manager and the HOD get the Approvals queue and nothing
else, and the PA gets their own run of pages with **Status** beside it. The `admin`
...
| 🧾 **An approved invoice files itself** | Nobody signs an invoice &mdash; it carries one signature, the consultant's, already on it when it is sent &mdash; so the HOD approving one is the last thing that happens to it. It finishes there rather than sitting in the PA's queue waiting on a signature that does not exist, and the invoice **as it was approved** is filed on the spot. Without that it never reached the list Group People &amp; Finance collect from, because that list is the copies on file. An invoice approved before this existed gets a **File it** button on the Status row |
| ✒️ **Two pages for the PA** | The PA's part is a signature on paper, so their run is the paper going out and coming back &mdash; they get **Download** (every time sheet the HOD has approved and not yet signed, one button each and *Download all*) and **Upload** (the same list, each with a box for the signed scan). Putting a scan on a card is not sending it: it can be read in the viewer and taken off again. One **Submit** at the bottom files every copy, closes those months and hands them to Group People &amp; Finance &mdash; scan first, month second, because a month closed with the scan lost to a failed upload is a month nobody can produce. A month already sent can be given a newer scan the same way, and **the newest copy is the one everybody sees** from then on **Status** sits beside that run: one of its stages is theirs, and a month is on that table before it reaches them |
| 🧭 **The numbers stop where the work stops** | Status is not something you do after Submit &mdash; it is where you go to see where Submit got to. Numbering it made a question look like a stage and the run look longer than the work. **Status** and **History** sit apart at the end of the bar now: same buttons, no numbers, no line joining them to anything, and a *Back to the form* at the bottom instead of a Next. The step counter counts the work, not the bar |
| 📒 **The PA sees what went through, with the leave** | Download and Upload are the job in front; **Filed** is the job behind &mdash; every signed time sheet sent on from there, a row each, newest first. Each row carries **the leave that month**, read off the very form the copy was filed against rather than typed anywhere, so nobody can get it wrong and nobody has to. Read it in the viewer or take a copy, from the row |
| 🗜️ **Download all is one archive** | It used to hand the browser a file at a time, a quarter of a second apart, and the browser asked whether it could save several &mdash; say no by accident and half a year of signed claims went nowhere. Now it fetches everything listed and saves **one zip**, named for whoever and whatever it was narrowed to. The writer is a page of code in `zip.js` rather than another library in `vendor/`: these files are scanned PDFs and JPEGs, already compressed, so the archive stores rather than squeezes |
| 📥 **Download all, for the whole record** | A claim finishes with the PA, and nobody collects it afterwards. What it leaves behind is the administrator's: **History** lists every signed form on file, for everybody, and **Download all** saves the lot as one zip, each file named for the person and the month rather than for whatever the scanner called it |
...
│       ├── signing.js            # the PA's two pages: Download, then Upload
│       ├── zip.js                # the archive Download all builds, written here
